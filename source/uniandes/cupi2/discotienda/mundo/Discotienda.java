...
public class Discotienda
{
    ...
    public void RegistrarDiscos() throws IOException, ElementoExisteException{
 		File archivo = new File("./data/DiscosParaSubir.txt");
    ...
    /**
    * Crear un informe de canciones con sus discos
    * @param numeroCanciones 
    * @throws FileNotFoundException 
    * @throws IOException 
    */
   public void reporteDiscos(int Canciones) throws FileNotFoundException 
   {  	
   	File archivo = new File("./data/reporteDeDiscos.txt");
   	PrintWriter pluma = new PrintWriter(archivo);
   	pluma.println("REPORTE DE DISCOS CON MENOS DE "+Canciones+" CANCIONES");
   	pluma.println("------------------------------------------------------------");
   	for(int i=0;i<discos.size();i++)
   	{
   		Disco miDisco = (Disco)discos.get(i);
   		if(miDisco.darNombresCanciones().size()<Canciones && miDisco.darNombresCanciones().size()>0)
   		{
   	        pluma.println("");
   			...
   			for(int j=0;j<miDisco.darNombresCanciones().size();j++)
   			{
   				...
   				pluma.println("Nombre: " + miCancion.darNombre() + "     Precio: " + miCancion.darPrecio() + "     Unidades Vendidas: " + miCancion.darUnidadesVendidas());
   				pluma.println("---------------------------------------------------------------------------------");
   			}
   			pluma.println("");
   		}
    ...
    {
    	 try {
         	String entrada = JOptionPane.showInputDialog("Ingrese el numero de canciones a buscar");
         	int numeroUsuario = Integer.parseInt(entrada);
         	reporteDiscos(numeroUsuario);
         	return "reporte generado satisfactoriamente!";
 		} catch (FileNotFoundException e) {
 			return "Error fatal: "+ e.getLocalizedMessage();
 		}
    }
